...
# Set page config
st.set_page_config(page_title="XYZ Indicator Dashboard", layout="wide", initial_sidebar_state="collapsed")
...
with col2:
    st.selectbox("", ["December 2025", "November 2025", "October 2025"], key="month_selector")

months, comp_data = generate_sample_data()
...
with col3:
    st.markdown("""
    <div class="metric-card" style="border-left: 4px solid #16a085;">
...
with col4:
    st.markdown("""
    <div class="metric-card" style="border-left: 4px solid #7f8c8d;">
        <h4>🔧 Manpower Fulfillment</h4>
...
with col2:
    ...
    delivery_fig = go.Figure(go.Indicator(mode="gauge+number", value=92, number=dict(font=dict(size=16)),
...
with col3:
    ...
    defect_fig = go.Figure(go.Indicator(mode="gauge+number", value=5, number=dict(font=dict(size=16)),
...
with col4:
    ...
    sla_fig = go.Figure(go.Indicator(mode="gauge+number", value=95, number=dict(font=dict(size=16)),
                       gauge={'axis': {'range': [None, 100]}, 'bar': {'color': "#27ae60"}}))
...
with col1:
    ...
    dep_fig = go.Figure(go.Scatter(x=months, y=[85, 87, 89, 91, 88, 92, 94, 96, 93, 95, 97, 98], mode='lines+markers'))
...
with col2:
    ...
    sit_fig = go.Figure(go.Bar(x=months, y=[78, 82, 85, 88, 91, 89, 93, 96, 94, 97, 95, 98], marker_color='#16a085'))
...
with col3:
    ...
    turnover_fig = go.Figure(go.Indicator(mode="gauge+number", value=7, number=dict(font=dict(size=16))))
